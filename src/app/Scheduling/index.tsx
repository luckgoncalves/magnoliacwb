export function Scheduling() {
  return (
    <section className="flex flex-col items-start justify-center bg-[#FFCDBC] rounded-[50px] mt-20 w-full">
      <div className=" w-full flex justify-between items-center p-20 ">
        <h2 className="text-4xl" >Agende um horário</h2>
        <a
        href="https://wa.me/5511989817883?text=Ol%C3%A1,%20gostaria%20de%20agendar%20um%20hor%C3%A1rio"
        target="_blank"
        rel="noreferrer"
        className="bg-[#526B53] py-5 px-20 rounded-lg text-[#FEC89A] hover:opacity-80 font-light">Whatsapp</a>
      </div>
    </section>

  )
}
